...
第一次调用浏览器工具时会打开独立 Chrome

后续 Codex 任务连接同一个独立 Chrome，并在各自标签页中工作，不再重复启动占用同一资料的浏览器进程
...
| 运行控制与临时输出 | `~/Library/Caches/AIALRA Shopping Browser/MCP` | 保存浏览器单例状态；每个客户端的日志、错误页、截图和下载进入独立临时目录，客户端结束后删除 |
...
- 所有任务共享一个独立浏览器资料，每个任务只使用自己的一个标签页序列
- 新任务通过本机回环 CDP 连接已经运行的独立 Chrome，避免资料目录占用冲突
- Chrome 仍在运行但临时端口记录丢失时，启动器会校验本机进程、资料目录和健康端点，再恢复现有连接
- 任务不得关闭或改写其他任务的标签页
- 平台搜索与详情默认串行执行
- 相邻自动动作至少间隔三秒
...
- 每个客户端使用权限仅限当前用户的临时输出目录，客户端结束时完整删除
- 异常退出留下的客户端临时目录由后续启动器确认原进程结束后清理
- 原始页面结构不进入长期缓存，结构化证据保存前必须清除临时参数
